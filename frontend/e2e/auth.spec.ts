...
test.describe('Authentication', () => {
  test('login page shows email and guest PIN sections', async ({ page }) => {
  ...
  test('email login sends magic link and shows success message', async ({ page }) => {
    ...
    await expect(page.getByText(/odkaz|odesláno|poslán/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('guest cannot create tasks - no "Pridat ukol" button on board', async ({ page }) => {
    await page.request.post('http://localhost:5000/api/auth/guest', {
      data: { pin: 'ovcina2026' },
    });
    await page.goto('/board');
    await expect(page.getByText('Přidat úkol')).not.toBeVisible({ timeout: 5000 });
  });
